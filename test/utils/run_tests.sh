...
if [ "${TARGET}" = "sanity" ]; then
    ...
    if test x"$TOXENV" = x'py24' ; then python2.4 -V && python2.4 -m compileall -fq -x 'module_utils/(a10|rax|openstack|ec2|gce|dimensiondata|docker_common|azure_rm_common|vca|vmware).py' lib/ansible/module_utils ; fi
else
